---
title: "Aira_caryophyllea - Preparing Global Model Inputs"
...
install.packages('terra', repos='https://rspatial.r-universe.dev')
  library(terra)
p_load("dplyr", "tidyr", "ggplot2", "terra",  "sf", "sp", "maptools", "CoordinateCleaner",  "spThin", "modeest", "beepr", "rgdal", "data.table")
```
```{r}
# setting paths
  setwd("D:/Codingwork/Fire")
  path.root <- "D:/Codingwork/Fire"  # typical class root dir
...
```
```{r}
pres$Species <- gsub("Aira_caryophyllea", 1, pres$Species)
head(pres, 2)
```
...
```{r}
kms_to_thin_by = 20
output <- spThin::thin(pres, lat.col = 'decimalLatitude', long.col = 'decimalLongitude', spec.col = "X", thin.par = (kms_to_thin_by ), reps = 3, locs.thinned.list.return = TRUE, write.files = FALSE, verbose = TRUE)
beepr::beep(2)
```

Since spThin did 3 iterations, there are 3 different variations of
...
```
```{r}
local.pres <- st_join(pres, islandpolys, join = st_intersects, left = FALSE ) %>% dplyr::select(X, Species, decimalLongitude, decimalLatitude, isle,	geometry)
...
global.pres <- st_join(pres, islandpolys, join = st_intersects,  left = TRUE ) %>%
  subset(is.na(isle)) %>%
  dplyr::select(X, Species, decimalLongitude, decimalLatitude, isle,	geometry)
...
```{r}
global.presFNET <- global.presFNET %>% dplyr::select("FNETID",  "decimalLongitude", "decimalLatitude", "Species")
...
```
```{r}
table(p1$Species)[[1]] # N spp locations
```
```{r}
dim(p3)[1] + table(p1$Species)[[1]] # should equal N of FISHNET
```
```{r}
...
```
```{r}
dim(p4)[1] + table(p1$Species)[[1]] # should equal N modelling frame
```
```{r}
# recall ...
table(p1$Species)[[1]] # N of spp locations
```
```{r}
...
pseu.srs2 <- pseu.bufpt[sample(1:nrow(pseu.bufpt), 3 * table(p1$Species)[[1]], replace = F), ]
...
library(terra)
setwd(path.in.general)
baseline <- list.files(path = paste(path.in.general, "/wc2.1_30s_bio/",  sep = ""), full.names = T)
baseline
```
...
#Using spearman because it has no assumption of normality
  ...
  c1 <- cor(tr.data[, c(8:27)], use = "pairwise.complete.obs", method = "spearman") # est. correlation
```
```{r}
...
```
```{r}
keep <- c("bio2", "bio3", "bio16", "bio15", "alt")
...
boxplot(tr.picked[,length(tr.picked) - length(keep) + 5 ] ~ tr.picked$Species, xlab = "Presence:Absence",
    ylab = keep[6])

boxplot(tr.picked[,length(tr.picked) - length(keep) + 6 ] ~ tr.picked$Species, xlab = "Presence:Absence",
    ylab = keep[7])
...
tr.picked %>% 
  ...
  ggplot() +
  geom_density(aes(x = value, y = ..density.., color = factor(Species))) +
